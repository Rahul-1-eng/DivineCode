...
interface TestCase {
  ...
  actualOutput: string;
  error: string;
  verdict: 'PENDING' | 'ACCEPTED' | 'WRONG_ANSWER' | 'COMPILATION_ERROR' | 'RUNTIME_ERROR' | 'JUDGE_ERROR' | '';
}
...
export default function StandaloneJudgePage() {
  ...
  async function runTestCase(tc: TestCase) {
    try {
      const response = await fetch(`${API_V2_BASE_URL}/execute`, {
        method: 'POST',
      ...
      } else if (data.verdict === 'Accepted' || data.verdict === 'ACCEPTED') {
        verdict = 'ACCEPTED';
      } else if (data.verdict === 'Wrong Answer' || data.verdict === 'WRONG_ANSWER') {
        verdict = 'WRONG_ANSWER';
      } else if (data.verdict?.includes('Error') || data.verdict?.includes('Exceeded')) {
        verdict = 'RUNTIME_ERROR';
        if (!error) error = data.message || 'Runtime error occurred';
      } else {
        verdict = 'JUDGE_ERROR';
        if (!error) error = data.verdict || 'Unknown judge state';
      }
