...
export default function CreateContestPage() {
  ...
  // 👉 NEW: State to hold fetched Database MCQs
  ...
  async function lookupProblem(index: number) {
    ...
    if (p.platform === 'Interview MCQ') return; // MCQs are handled by the dropdown
  ...
  async function createContest() {
    ...
    const contestProblems = problems.map((p) => ({ 
      title: p.title, 
      platform: p.platform, 
      code: p.code || `${p.contestCode}${p.problemIndex}`, 
      ...
      tags: p.tags,
      interviewQuestionId: p.interviewQuestionId // 👉 Included for Backend mapping
    })).filter((p) => p.url);
    ...
    try {
      const res = await fetch(`${API_V2_BASE_URL}/contests`, {
        ...
        body: JSON.stringify({ 
          title, 
          description: `${mode === 'single' ? 'Solo' : 'Team'} mashup created by ${ownerName}`, 
            ...
            {problems.map((p, i) => (
              <div key={i} style={problemCard}>
                ...
                <select value={p.platform} onChange={(e) => updateProblem(i, 'platform', e.target.value)} style={{ ...smallInput, flex: '1 1 120px' }}>
                  ...
                  {/* 👉 NEW OPTION */}
                  <option>Interview MCQ</option> 
                </select>

                {/* 👉 DYNAMIC INPUT: Textbox for coding platforms, Dropdown for MCQs */}
                {p.platform === 'Interview MCQ' ? (
                  <select 
                    value={p.interviewQuestionId || ''} 
                    onChange={(e) => {
                      const qId = e.target.value;
                      const q = availableMcqs.find(m => m.id === qId);
                      if (q) {
                        updateProblem(i, 'interviewQuestionId', q.id);
                        updateProblem(i, 'code', q.id); // Set code as ID for fallback lookup
                        updateProblem(i, 'title', q.title || q.prompt.substring(0, 50));
                        updateProblem(i, 'url', `/interview`);
                        updateProblem(i, 'tags', 'mcq, theory');
                        updateProblem(i, 'difficulty', q.difficultyLabel || 'Medium');
                        setLookupState({ ...lookupState, [i]: 'MCQ loaded directly from database.' });
                      }
                    }}
                  ...
                  >
                    ...
                    {availableMcqs.map(m => (
                      <option key={m.id} value={m.id}>{m.title || m.prompt.substring(0, 40)}...</option>
                    ))}
                  </select>
                ) : (
                ...
                <button onClick={() => lookupProblem(i)} disabled={p.platform === 'Interview MCQ'} style={{ ...ghostBtn, flex: '1 1 100px', opacity: p.platform === 'Interview MCQ' ? 0.5 : 1 }}>
                  Lookup
                </button>
                ...
                <small style={{ color: '#94a3b8', width: '100%', marginTop: 4 }}>
                  {lookupState[i] || 'Lookup fills title, URL, rating, contest/index. Codeforces additions are rejected if any player already solved them.'}
                </small>
              </div>
            ))}
          </div>
