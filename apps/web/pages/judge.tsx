...
import { recomputeContestStandings } from '../../api/src/modules/standings/standingService'

// We are using the 100% Free Wandbox API for backend execution
...
export async function judgeQueuedSubmission(submissionId: string) {
  const submission = await prisma.submission.findUnique({
    ...
    include: {
      ...
      contestProblem: true,
      participant: true // 👉 Added to fetch the teamId for anti-cheat
    }
  });
  ...
  // 👉 NEW: Anti-Cheat / Spam Protection
  // Prevent users from spamming the exact same code, or teammates copy-pasting the same code.
...
// ---------------------------------------------------------
// 2. LIVE EDITOR PATH (For Custom Input / Output)
// ---------------------------------------------------------
