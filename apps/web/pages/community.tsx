...
    setLoading(true);
    try {
      const data = await fetchApi('/api/v2/community/problems');
...
    setUploading(true);
    try {
      // Fetch user ID using email to attach as author
      const profile = await fetchApi('/api/v2/profile/me');
      
      const payload = {
        userId: profile.id,
        title: newPost.title,
...
      await fetchApi('/api/v2/community/upload', {
        method: 'POST',
...
                  <h3 style={{ margin: '0 0 8px 0', color: '#eef2ff', fontSize: 18 }}>{post.title}</h3>
