...
- AI Recruiter (/recruiter): a 3-round mock interview — DSA with live code execution, then a resume-based deep dive, then behavioral. Every user gets 3 free interviews after verifying their mobile number with an OTP (one number = one set of trials); after that a session costs 100 coins, less on lucky days or with a coupon code. Uploading a resume PDF personalizes Round 2. When the loop ends, a hiring-committee report with scores, strengths, weaknesses and a prep plan is available and can be downloaded as a PDF.
- Real Recruiter marketplace (/recruiter/book): book a paid 1:1 LIVE VIDEO mock interview with a verified human recruiter. Two ways to pay: online (card / UPI / netbanking / wallet — instant), or manual UPI to the platform handle + submit the UTR which the admin verifies. Total = recruiter fee + platform fee (15%, minimum Rs.49; the platform-fee part is discounted on lucky days, the recruiter's fee never is). After payment the recruiter confirms they are free for the slot — usually with a quick phone call to the candidate — and only then does the private live video room unlock ("Join Live Interview" on the bookings page; the link is also emailed to both sides). "Payment Under Verification" = admin has not checked the UTR yet; "Awaiting Recruiter Confirmation" = paid, recruiter has not accepted the slot yet; if the recruiter declines, the admin settles the refund. Users can also apply to become a recruiter on the same page; listings go live after admin approval.
- Contests: timed rated contests with live leaderboards; registering sends a confirmation email, and every user is emailed when a new contest is created. Duel: real-time 1v1 code battles.
- Community: video tutorials and walkthroughs posted by users, plus DivineLive — go live camera-on from the Community Hub to discuss contest problems, and anyone online can join the room on video. Everyone gets an email when a stream starts or a tutorial is uploaded.
- Emails: booking updates, recruiter application decisions, interview debrief reports, contest registrations, coin purchase receipts and OTPs (when SMS is unavailable) go to the account's registered email.
- Coins: earned by solving problems and platform activity, or bought at /coins — the base pack is 50 coins for Rs.10, bigger packs carry a bulk bonus. Pay online or by manual UPI + UTR (admin verifies, then coins land). Admin coupon codes give a percent off coin packs or AI sessions (one use per person); the better of coupon vs lucky-day discount applies — they never stack.
- Lucky days: some days the platform randomly runs a sitewide deal — discounted or even free AI interviews, cheaper coin packs, reduced platform fee on bookings. The banner on /coins and /recruiter shows today's deal; it resets at midnight IST.
